...
function renderSunburst(data, year) {
    ...
    // Add the title.
    svg.append("text").attr("font-size", 16).attr("fill", "#FFF").attr("text-anchor", "middle").attr("transform", "translate(0, " + (-10 - height / 2) + ")").text("Coffee Production by Country in thousand 60kg bags");
    ...
    function mouseover(d) {
        tooltip.text(d.key + ": " + d.value + " thousand 60kg bag" + (d.value > 1 ? "s" : "")).transition().attr("fill-opacity", 1);
    };
    ...
    // Enter
    path.enter().append("path").attr("stroke", "bar").attr("fill-rule", "evenodd");

    // Update
    path.attr("d", arc).attr("stroke", "#FFF").attr("fill-rule", "evenodd").attr("fill", color).on("click", click).on("mouseover", mouseover).on("mouseout", mouseout);
